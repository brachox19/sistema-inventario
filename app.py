...
# ---------------------------------------------------------
# CONEXIÓN A LA BASE DE DATOS EN LA NUBE (TURSO) - PASO 5
# ---------------------------------------------------------
try:
  TURSO_URL = st.secrets["libsql://inventario-vps-brachox19.aws-us-west-2.turso.io"]
  TURSO_TOKEN = st.secrets["token:eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3ODk4Njc1OTcsImlkIjoiMDFhMGJjNmItNWUwMS03YTQ5LWIyNzUtNDVmNWVmNzZmMjdmIiwia2lkIjoiNHZndmFuRWwwLU42NXV0eUpHdGZwMUhIaVpYTTJ5djhpU1ZoMmQ2QnZObyIsInJpZCI6ImM0ZTQ2NGRhLTM0YjAtNDI1Zi04NTBlLTAxM2U4OWVjOWU5YyJ9.hXZM0Y0Tw6hNBtjM6Pc8ugu1A_4ub0_ycGYjUwVIQYuMyg4lhwA6E1uF1YDEkH_HoeW6tJvO-Mhjiu43BC0RBw"]
...
def login():
  ...
  if st.button("Ingresar"):
    if username == "admin" and password == "admin123":
      ...
      st.rerun()
    elif username == "isel" and password == "isell123":
      st.session_state["logged_in"] = True
      st.session_state["user_role"] = "Isell"
      st.session_state["username"] = "Isell"
      st.rerun()
    else:
...
if not st.session_state["logged_in"]:
  ...
  if opcion == "🛒 Punto de Venta":
    ...
    prods = pd.read_sql_query(
        "SELECT id, codigo, nombre, precio, stock FROM productos WHERE stock >"
        " 0",
        conn,
    )
    clientes_df = pd.read_sql_query(
        "SELECT cedula, nombre FROM clientes", conn
    )
    ...
    with col1:
      ...
      if not prods.empty:
        opciones_prods = {
            f"{row['nombre']} | Cód: {row['codigo']} | ${row['precio']} (Disponibles: {row['stock']})": row
            for _, row in prods.iterrows()
        }
        prod_selected_str = st.selectbox(
            "Escribe o selecciona un producto:", list(opciones_prods.keys())
        )
          ...
          if st.button("➕ Agregar al Carrito", use_container_width=True):
            if cant > stock_restante_para_carrito:
              if stock_restante_para_carrito <= 0:
                st.error(
                    f"❌ Stock agotado en carrito. ({stock_disponible} en"
                    " inventario)"
                )
              else:
                st.error(
                    f"❌ Solo puedes agregar {stock_restante_para_carrito} más."
                )
            else:
        ...
        st.warning(
            "⚠️ No hay productos con stock disponible en este momento."
        )
    ...
    with col2:
      ...
      if st.session_state["carrito"]:
        ...
        cliente_nombre = st.selectbox(
            "Seleccionar Cliente:", ["Cliente General"] + list(clientes_df["nombre"])
        )
            ...
            if submit_cli:
              if new_cedula and new_nombre:
                try:
                  ...
                  conn.commit()
                  st.success(
                      "✅ Guardado. Ahora selecciónalo en la lista."
                  )
                  st.rerun()
                except Exception:
        ...
        tipo_pago = st.radio(
            "Forma de Venta:", ["Contado", "Crédito"], horizontal=True
        )

        metodo_pago = "Crédito"
        if tipo_pago == "Contado":
          metodo_pago = st.selectbox(
              "Método de Pago:", ["Pago Móvil", "Efectivo", "Zelle", "Binance"]
          )
        ...
        if st.button(
          ...
          c.execute(
              "INSERT INTO ventas (fecha, cliente, tipo_pago, metodo_pago,"
              " total, vendedor) VALUES (?, ?, ?, ?, ?, ?)",
              (
                  fecha,
                  cliente_nombre,
                  tipo_pago,
          ...
          for item in st.session_state["carrito"]:
            c.execute(
                "INSERT INTO detalle_ventas (venta_id, producto, cantidad,"
  ...
  # ----------------------------------------------------
    ...
    if st.session_state["user_role"] == "Admin":
      tab1, tab2, tab3 = st.tabs(
          ["📋 Lista de Productos", "➕ Registrar Producto", "✏️ Editar / 🗑️ Eliminar"]
      )
    else:
    ...
    with tab1:
      ...
      if st.session_state["user_role"] == "Admin":
        df_prods = pd.read_sql_query(
            "SELECT id, codigo, nombre, categoria, precio, costo, stock FROM"
            " productos",
            conn,
        )
      else:
    ...
    if st.session_state["user_role"] == "Admin":
      with tab3:
        ...
        if not df_prods.empty:
          opciones = {
              f"{r['id']} - {r['nombre']} (Cód: {r['codigo']})": r
              for _, r in df_prods.iterrows()
          }
          prod_sel_key = st.selectbox(
              "Seleccionar producto a gestionar", list(opciones.keys())
          )
          ...
          with col_e1:
            ...
            edit_codigo = st.text_input(
                "Código", value=str(prod_data["codigo"]), key="edit_cod"
            )
            edit_nombre = st.text_input(
                "Nombre", value=str(prod_data["nombre"]), key="edit_nom"
            )
            edit_cat = st.text_input(
                "Categoría", value=str(prod_data["categoria"]), key="edit_cat"
            )
            edit_precio = st.number_input(
                "Precio ($)",
                value=float(prod_data["precio"]),
                format="%.2f",
                key="edit_prec",
            )
            edit_costo = st.number_input(
                "Costo ($)",
                value=float(prod_data["costo"]),
                format="%.2f",
                key="edit_cost",
            )
  ...
  # ----------------------------------------------------
    ...
    if st.session_state["user_role"] == "Admin":
      tab1, tab2, tab3 = st.tabs(
          ["📋 Lista de Clientes", "➕ Registrar Cliente", "✏️ Editar / 🗑️ Eliminar"]
      )
    else:
    ...
    if st.session_state["user_role"] == "Admin":
      with tab3:
        ...
        if not df_clientes.empty:
          opciones_c = {
              f"{r['id']} - {r['nombre']} (ID: {r['cedula']})": r
              for _, r in df_clientes.iterrows()
          }
          cli_sel_key = st.selectbox(
              "Seleccionar cliente a gestionar", list(opciones_c.keys())
          )
          ...
          with col_c1:
            ...
            edit_cedula = st.text_input(
                "Cédula / ID", value=str(cli_data["cedula"]), key="edit_ced"
            )
            edit_cnombre = st.text_input(
                "Nombre", value=str(cli_data["nombre"]), key="edit_cnom"
            )
            edit_tel = st.text_input(
                "Teléfono", value=str(cli_data["telefono"]), key="edit_ctel"
            )
            edit_dir = st.text_area(
                "Dirección", value=str(cli_data["direccion"]), key="edit_cdir"
            )

            if st.button("💾 Actualizar Cliente"):
              c.execute(
                  "UPDATE clientes SET cedula=?, nombre=?, telefono=?, direccion=?"
                  " WHERE id=?",
                  (
                      edit_cedula,
                      edit_cnombre,
  ...
  # ----------------------------------------------------
    ...
    if not df_v.empty:
      ...
      st.caption(
          "Al borrar una venta, los productos vendidos serán devueltos"
          " automáticamente al stock."
      )

      venta_cancel_sel = st.selectbox(
          "Seleccionar Venta a Anular",
          df_v["id"].astype(str)
          + " - Fecha: "
          + df_v["fecha"]
          + " - Cliente: "
      ...
      if st.button("🗑️ Anular y Revertir Venta"):
        detalles = c.execute(
            "SELECT producto, cantidad FROM detalle_ventas WHERE venta_id=?",
            (venta_id_to_del,),
        ).fetchall()
        ...
        c.execute(
            "DELETE FROM cuentas_por_cobrar WHERE venta_id=?", (venta_id_to_del,)
        )
  ...
  # ----------------------------------------------------
    ...
    df_creditos = pd.read_sql_query(
        "SELECT SUM(monto_pendiente) as total_credito FROM cuentas_por_cobrar"
        " WHERE estado = 'Pendiente'",
        conn,
    )
    total_credito_pendiente = (
        df_creditos["total_credito"].values[0]
        if not df_creditos.empty
        and df_creditos["total_credito"].values[0] is not None
        else 0.0
    )
  ...
  # ----------------------------------------------------
    ...
    st.warning(
        "⚠️ Atención: Las siguientes acciones eliminarán la información de la"
        " base de datos de manera irreversible."
    )
    ...
    with col1:
      ...
      st.write(
          "Elimina solo el registro de ventas, cuentas por cobrar y el historial"
          " financiero, manteniendo tus productos y clientes intactos."
      )
    ...
    with col2:
      ...
      st.write(
          "Borra **TODA** la información: productos del inventario, lista de"
          " clientes, historial de ventas y deudas."
      )
